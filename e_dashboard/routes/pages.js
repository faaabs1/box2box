...
router.get('/', async (req, res) => {
  try {
    ...
    res.render('league', { leagues, seasons, table, teams, selectedLeague: leagueId, selectedSeason: seasonId, myTeamId: db.MY_TEAM_ID, page: 'league' });
  } catch (e) {
...
router.get('/team', async (req, res) => {
  try {
    ...
    const [games, stats, timing, leagueAvgs] = await Promise.all([
      db.fetchTeamGames(teamId, seasonId, leagueId),
      db.fetchTeamSeasonStats(teamId, seasonId, leagueId),
      db.fetchTeamGoalsTiming(teamId, seasonId),
    ...
    res.render('team', { leagues, seasons, allTeams, games, stats, timing, leagueAvgs, gamesPlayed, selectedTeam, selectedTeamId: teamId, selectedSeason: seasonId, selectedLeague: leagueId, myTeamId: db.MY_TEAM_ID, page: 'team' });
  } catch (e) {
    res.status(500).render('error', { message: e.message });
  }
});
